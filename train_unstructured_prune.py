...
def reset_batchnorm(model, loader):
    for module in model.modules():
        if isinstance(module, (torch.nn.BatchNorm1d, torch.nn.BatchNorm2d, torch.nn.BatchNorm3d)):
            # Reset running mean and running variance
            module.running_mean.zero_()
            module.running_var.fill_(1)
    # Switch model to training mode to update batchnorm statistics
    model.train()

    # Cycle through the loader if it has fewer than 100 batches
...
for step in range(total_steps):
    ...
    if step in record_steps:
        ...
        total = 0
        total_loss = 0
        # reset_batchnorm(model, train_loader)
        model.eval()
        for img, labels in test_loader:
            img = img.to(device)
            labels = labels.to(device)
