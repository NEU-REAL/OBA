...
parser.add_argument("--sl-num-epochs", type=int, default=100, help="epochs for sparsity learning")
parser.add_argument("--sl-lr", default=0.001, type=float, help="learning rate for sparsity learning")
parser.add_argument("--sl-lr-decay-milestones", default="50,75", type=str, help="milestones for sparsity learning")
parser.add_argument('--batch_size', default=64, type=int, help='batch size')
parser.add_argument('--use_wandb', default=False, type=bool, help='whether to record data on wandb')
...
def get_dataset(dataset):
    ...
    if dataset == "cifar10":
        train = torchvision.datasets.CIFAR10(root=dataset_dirs['cifar10'], train=True, transform=train_transform, download=True)
        test = torchvision.datasets.CIFAR10(root=dataset_dirs['cifar10'], train=False, transform=val_transform, download=True)
        return train, test
    elif dataset == "cifar100":
        train = torchvision.datasets.CIFAR100(root=dataset_dirs['cifar100'], train=True, transform=train_transform, download=True)
...
tiny_test = torch.utils.data.Subset(test, indices)
tiny_loader = torch.utils.data.DataLoader(tiny_test, batch_size=25)
...
wrapped_pruner = tp.WrappedPruner(args, train_loader, test_loader, model, example_inputs, ignored_layers,
                           1 - 0.045, device, builder=builder)
...
raw_flops, raw_params = profile(model, inputs=(example_inputs,))
time_idx = 0
ops_percents = []
while prune_step < len(ops_ratio_list):
    if args.multistep:
        ...
        test_acc, test_loss = eval(model, tiny_loader, device=device)
        ops_percents.append(ops_percent)
